...
class InventoryListCreateView(APIView):
    ...
    def get(self, request: Request, *args, **kwargs) -> Response:
        request_date = request.query_params.get("created_at", None)

        if request_date is not None:
            corrected_date = datetime.strptime(request_date, "%Y-%m-%d")

            raw_query_set = self.get_queryset()
            filtered_queryset = raw_query_set.filter(created_at__gt=corrected_date)
